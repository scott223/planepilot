    ...
    loop {
        ...
        if &buf[0..4] == b"DATA" {
            for sentence in buf[5..len].chunks(36) {
                ...
                // Ese the values and datamap to make a hashmap that contains key-value pairs for the state
                let values = map_values(sentence[0], values, &datamap::data_map())
                    .map_err(|e| {
                        event!(
                            Level::ERROR,
