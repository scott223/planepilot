...
impl AppState {
    ...
    pub async fn process(mut self) {
        while let Some(signal) = self.receiver.recv().await {
            match signal {
                StateSignal::SetFlying { are_we_flying, result_sender } => {
                ...
                StateSignal::SetPlaneState { plane_state, result_sender } => {
                ...
                StateSignal::SetStandbyHeading { standby_heading, result_sender } => {
                ...
                StateSignal::ActivateStandbyHeading { result_sender } => {
                    self.auto_pilot_state.horizontal_guidance.heading_setpoint = self.auto_pilot_state.horizontal_guidance.heading_standby;
                    let _ = result_sender.send(true);
                }
                StateSignal::SetHorizontalGuidanceToStandbyMode { result_sender } => {
                    self.auto_pilot_state.horizontal_guidance.horizontal_mode = HorizontalModes::Standby;
                    let _ = result_sender.send(true);
                }
                StateSignal::SetHorizontalGuidanceToWingsLevelMode { result_sender } => {
                    self.auto_pilot_state.horizontal_guidance.horizontal_mode = HorizontalModes::WingsLevel;
                    let _ = result_sender.send(true);                    
                }
                StateSignal::SetHorizontalGuidanceToHeadingMode { result_sender } => {
                    self.auto_pilot_state.horizontal_guidance.horizontal_mode = HorizontalModes::Heading;
                    let _ = result_sender.send(true);
                }
                StateSignal::SetStandbyVelocity { standby_velocity, result_sender } => {
                ...
                StateSignal::ActivateStandbyVelocity { result_sender } => {
                    self.auto_pilot_state.vertical_guidance.velocity_setpoint = self.auto_pilot_state.vertical_guidance.velocity_standby;
                    let _ = result_sender.send(true);
                }
                StateSignal::SetStandbyAltitude { standby_altitude, result_sender } => {
                ...
                StateSignal::ActivateStandbyAltitude { result_sender } => {
                    self.auto_pilot_state.vertical_guidance.altitude_setpoint = self.auto_pilot_state.vertical_guidance.altitude_standby;
                    let _ = result_sender.send(true);
                }
                ...
                StateSignal::SetVerticalGuidanceToTECSMode { result_sender } => {
                    self.auto_pilot_state.vertical_guidance.energy_error_integral = 0.0;
...
impl AppStateProxy {
    ...
    pub async fn set_flying(&self, are_we_flying: bool) -> anyhow::Result<()> {
        ...
        self.state_sender
            .send(StateSignal::SetFlying { are_we_flying, result_sender })
            .await?;

        match result_receiver.await.unwrap_or_else(|_| panic!("Failed to receive result from auto pilot state")) {
            true => return Ok(()),
    ...
    pub async fn set_plane_state(&self, plane_state: HashMap<String,Value>) -> anyhow::Result<()> {
        ...
        self.state_sender
            .send(StateSignal::SetPlaneState { plane_state, result_sender })
            .await?;
    ...
    pub async fn set_heading_standby(&self, heading: f64) -> anyhow::Result<()> {
        let (result_sender, result_receiver) = oneshot::channel();
        self.state_sender
            .send(StateSignal::SetStandbyHeading { standby_heading: heading, result_sender: result_sender })
            .await?;
    ...
    pub async fn activate_heading_setpoint(&self) -> anyhow::Result<()> {
        let (result_sender, result_receiver) = oneshot::channel();
        self.state_sender
            .send(StateSignal::ActivateStandbyHeading { result_sender: result_sender })
            .await?;
    ...
    pub async fn activate_horizontal_standby_mode(&self) -> anyhow::Result<()> {
        let (result_sender, result_receiver) = oneshot::channel();
        self.state_sender
            .send(StateSignal::SetHorizontalGuidanceToStandbyMode { result_sender: result_sender })
            .await?;
    ...
    pub async fn activate_horizontal_wingslevel_mode(&self) -> anyhow::Result<()> {
        let (result_sender, result_receiver) = oneshot::channel();
        self.state_sender
            .send(StateSignal::SetHorizontalGuidanceToWingsLevelMode { result_sender: result_sender })
            .await?;
    ...
    pub async fn activate_horizontal_heading_mode(&self) -> anyhow::Result<()> {
        let (result_sender, result_receiver) = oneshot::channel();
        self.state_sender
            .send(StateSignal::SetHorizontalGuidanceToHeadingMode { result_sender: result_sender })
            .await?;
    ...
    pub async fn set_velocity_standby(&self, velocity: f64) -> anyhow::Result<()> {
        let (result_sender, result_receiver) = oneshot::channel();
        self.state_sender
            .send(StateSignal::SetStandbyVelocity { standby_velocity: velocity, result_sender: result_sender })
            .await?;
    ...
    pub async fn activate_velocity_setpoint(&self) -> anyhow::Result<()> {
        let (result_sender, result_receiver) = oneshot::channel();
        self.state_sender
            .send(StateSignal::ActivateStandbyVelocity { result_sender: result_sender })
            .await?;
    ...
    pub async fn set_altitude_standby(&self, altitude: f64) -> anyhow::Result<()> {
        let (result_sender, result_receiver) = oneshot::channel();
        self.state_sender
            .send(StateSignal::SetStandbyAltitude { standby_altitude: altitude, result_sender: result_sender })
            .await?;
    ...
    pub async fn activate_alititude_setpoint(&self) -> anyhow::Result<()> {
        let (result_sender, result_receiver) = oneshot::channel();
        self.state_sender
            .send(StateSignal::ActivateStandbyAltitude { result_sender: result_sender })
            .await?;
    ...
    pub async fn activate_vertical_standby_mode(&self) -> anyhow::Result<()> {
        let (result_sender, result_receiver) = oneshot::channel();
        self.state_sender
            .send(StateSignal::SetVerticalGuidanceToStandbyMode { result_sender: result_sender })
            .await?;
    ...
    pub async fn activate_vertical_TECS_mode(&self) -> anyhow::Result<()> {
        let (result_sender, result_receiver) = oneshot::channel();
        self.state_sender
            .send(StateSignal::SetVerticalGuidanceToTECSMode { result_sender: result_sender })
            .await?;
